...
namespace Photon.Pun.UtilityScripts
{
    ...
    public class SmoothSyncMovement : Photon.Pun.MonoBehaviourPun, IPunObservable
    {
        ...
        private GameObject GameEnvironement;
        public void Awake()
        {
            bool observed = false;
            GameObject GameEnvironement = GameObject.Find("GameEnvParent");
        ...
        public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
        {
            if (stream.IsWriting)
            {
                ...
                stream.SendNext(transform.position - GameEnvironement.transform.position);
                stream.SendNext(transform.rotation);
            }
            else
            {
                //Network player, receive data
                correctPlayerPos = (Vector3)stream.ReceiveNext() + GameEnvironement.transform.position;
